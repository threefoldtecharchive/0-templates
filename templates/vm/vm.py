...
class Vm(TemplateBase):
    ...
    def _load_info(self, timeout):
        ...
        # if we don't have the vdisk url, yet get it from the vdisk service
        for disk in self.data["disks"]:
            if not disk["url"]:
                self._update_vdisk_url()
                break
        ...
        self.data["info"] = {
            ...
            "host": {
                "storage_addr": self._node_sal.storage_addr,
                "public_addr": self._node_sal.public_addr,
                "management_addr": self._node_sal.management_address,
            },
        }
        return self.data["info"]
    ...
    def add_portforward(self, name, target, source=None):
        ...
        forward = {"name": name, "target": target, "source": source}
        forward = self._populate_port_forwards([forward])[0]
        self.data["ports"].append(forward)

        self._node_sal.client.kvm.add_portfoward(self._vm_sal.uuid, forward["source"], forward["target"])
