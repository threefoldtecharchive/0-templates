...
class Vdisk(TemplateBase):
    ...
    def install(self):
        ...
        self.data["zerodb"], self.data["nsName"] = (
            node.schedule_action("create_zdb_namespace", kwargs).wait(die=True).result
        )
        ...
        disk = self._node_sal.primitives.create_disk(
            self.data["nsName"],
            zerodb_sal,
    ...
    def private_url(self):
        self.state.check("actions", "install", "ok")
        return (
            self._zerodb.schedule_action("namespace_private_url", args={"name": self.data["nsName"]})
            .wait(die=True)
            .result
        )
