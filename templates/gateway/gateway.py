...
class Gateway(TemplateBase):
    ...
    def _gateway_sal(self):
        gw = self.api.node_sal.primitives.from_dict("gateway", self.data)
        gw.name = self.guid
        return gw
    ...
    def add_dhcp_host(self, network_name, host):
        ...
        gateway_sal = self._gateway_sal
        host_sal = gateway_sal.networks[network_name].hosts.add(
            host["hostname"], host.get("ipaddress"), host.get("macaddress")
        )
