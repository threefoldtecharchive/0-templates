...
class TestNodeTemplate(ZrobotBaseTest):
    ...
    def test_node_info_node_running(self):
        ...
        node._node_sal.client.info.os.assert_called_once_with()
    ...
    def test_node_stats_node_running(self):
        ...
        node.state.set('status', 'running', 'ok')
        node.stats()
        node._node_sal.client.aggregator.query.assert_called_once_with()
    ...
    def test_reboot_node(self):
        ...
        """
        node = Node(name='node')
        node._node_sal.client.raw = MagicMock()
        node.reboot()

        node._node_sal.reboot.assert_called_with()
